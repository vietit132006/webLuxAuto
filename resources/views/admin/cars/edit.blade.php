...
@push('styles')
    @if (file_exists(public_path('build/manifest.json')) || file_exists(public_path('hot')))
        @vite('resources/css/admin-cars-edit.css')
    @endif
@endpush
...
@section('content')
    ...
    <div class="wrap lux-form-wrap">
        <div class="lux-page-header">
            <a class="admin-cars-edit-inline-13" href="{{ route('admin.cars.show', $car->car_id) }}"
                onmouseover="this.style.color='var(--accent)'" onmouseout="this.style.color='var(--muted)'">
                <svg class="admin-cars-edit-inline-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M11 15l-3-3m0 0l3-3m-3 3h8M3 12a9 9 0 1118 0 9 9 0 01-18 0z" />
                </svg>
            </a>
            <h1 class="lux-page-title">Chỉnh sửa: <span class="admin-cars-edit-inline-11">{{ $car->name }}</span></h1>
        </div>

        @if (session('success'))
            <div class="admin-cars-edit-inline-10">
                {{ session('success') }}
            </div>
        @endif
        @if (session('error'))
            <div class="admin-cars-edit-inline-9">
                {{ session('error') }}
            </div>
        @endif
        @if (session()->has('ui_log'))
            <div class="lux-card admin-cars-edit-inline-8">
                <h3 class="lux-card-title admin-cars-edit-inline-7">UI Debug Log</h3>
                <details>
                    <summary class="admin-cars-edit-inline-6">Mở log lần submit gần nhất
                    </summary>
                    <pre class="admin-cars-edit-inline-5">{{ json_encode(session('ui_log'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                </details>
            </div>
        @endif
        @if ($errors->any())
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stock_quantity" class="lux-input"
                            value="{{ old('stock_quantity', $car->stock_quantity ?? $car->stock ?? 0) }}" min="0" step="1">
                        <p class="field-hint">Nếu thay đổi, hệ thống sẽ tự ghi lịch sử tồn kho.</p>
            ...
            <div class="lux-card">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        @enderror
                        @if ($car->image)
                            <p class="admin-cars-edit-inline-2">Bỏ trống để giữ ảnh cũ.</p>
                        @endif
                    </div>
                ...
                @if ($car->images && $car->images->count())
                    <p class="existing-media-label admin-cars-edit-inline-4">Album hiện có ({{ $car->images->count() }}
                        ảnh):</p>
                ...
                <div class="form-row admin-cars-edit-inline-3">
                    <div class="form-group">
                        ...
                        @enderror
                        @if ($car->video_file)
                            <p class="admin-cars-edit-inline-2">
                                Video hiện tại: <a class="admin-cars-edit-inline-1" href="{{ asset('storage/' . $car->video_file) }}" target="_blank">Xem file</a> — bỏ trống để giữ.
                            </p>
                        @endif
                    </div>
